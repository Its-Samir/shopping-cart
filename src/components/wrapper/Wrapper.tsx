...
function Wrapper(props: { children: React.ReactNode }) {
  ...
  return (
    <div className="wrapper">
      <Navbar
        showCart={showDropdownCart}
        openCart={() =>
          setShowDropdownCart(
            (p) => window.location.pathname !== "/cart" && true
          )
        }
        closeCart={() => setShowDropdownCart(false)}
      />
      <div onMouseOver={() => setShowDropdownCart(false)}>{props.children}</div>
    </div>
  );
}
